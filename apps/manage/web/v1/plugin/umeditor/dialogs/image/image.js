(function () {
            ...
            $.each($imgs, function (index, node) {
                ...
                if (node.width > editor.options.initialFrameWidth) {
                    me.scale(node, editor.options.initialFrameWidth -
                        parseInt($(editor.body).css("padding-left")) - parseInt($(editor.body).css("padding-right")) );
                }

                return arr.push({
                    style: 'max-width: 100%;', //node.width
                    //height: node.height,
                    _src: node.src,
                    src: node.src
                });
    ...
    var Upload = {
        ...
        render: function (sel, t) {
			...
			$('.edui-image-file').attr('name', me.editor.getOpt('imageFieldName'));
        ...
        config: function (sel) {
            ...
            if(csrfToken){
              $(me.dialog).find('.J_csrf').html('<input type="hidden" name="_csrf" value="' + csrfToken + '" />');
            }
